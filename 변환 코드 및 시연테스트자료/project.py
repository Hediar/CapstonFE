...
def morphology(image):
    ...
    th1 = cv2.adaptiveThreshold(
        imgray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 15, 8)  # gaussian
    ...
    contours, high = cv2.findContours(
        imFlood, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    ...
    for contour in contours:
        ...
        resize = cv2.resize(cropped_image, dsize=(
...
def detect_text_fortext(path):
    ...
    for text in texts:
        ...
        verticesX = (['{}'.format(vertex.x)
                     for vertex in text.bounding_poly.vertices])
        verticesY = (['{}'.format(vertex.y)
                     for vertex in text.bounding_poly.vertices])
        ...
        print(text_number)
        textlist.append(
            [f'{w}.jpg', [verticesX[3], verticesY[2], xLength, yLength], textloc])
        print(textlist)
...
def makeppt():
    ...
    for i in range(2, int(text_number)+1):
        ...
        # k번째 Gvidata.json을 json_data로읽어들인다
        with open(f'/tmp/Txtdata{i}.json', 'r', encoding='UTF8') as file:
    ...
    for e in range(2, result_number+1):
        # k번째 Gvidata.json을 json_data로읽어들인다
        with open(f'/tmp/Gvidata{e}.json', 'r', encoding='UTF8') as file:
            contents = file.read()  # string
            data = json.loads(contents)
            x = int(data["location"][0])
            y = int(data["location"][1])
            ...
            if data['figure'] == "":
                ...
                if data['figure'] == "circle" and data['text'] == "":
                    print(data['figure'])
                    shape = shapes.add_shape(MSO_SHAPE.OVAL, Mm(
                        left), Mm(top), Mm(width), Mm(height))
                ...
                if data['figure'] == "square" and data['text'] == "":
                    print(data['figure'])
                    shape = shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Mm(
                        left), Mm(top), Mm(width), Mm(height))
                    shape.fill.background()
                    line = shape.line
                    line.color.rgb = RGBColor(0, 0, 0)
                if data['figure'] == "triangle" and data['text'] == "":
                    print(data['figure'])
                    shape = shapes.add_shape(MSO_SHAPE.ISOSCELES_TRIANGLE, Mm(
                        left), Mm(top), Mm(width), Mm(height))
                    shape.fill.background()
                    line = shape.line
                    line.color.rgb = RGBColor(0, 0, 0)
...
for l in range(text_number):
    ...
    print(json.dump(text_dictionary, open(
        '/tmp/' + f'Txtdata{l + 1}.json', 'w')))
...
for f in range(result_number):
    if f == 1:
        ...
        with open(f'/tmp/Gvidata{f + 1}.json', 'r', encoding='utf-8') as file:
            ...
            # 수정한 json데이터를 Gvidatak.json파일로 저장
            print(json.dump(json_data, open(
                '/tmp/' + f'Gvidata{f + 1}.json', 'w')))
