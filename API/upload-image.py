...
def lambda_handler(event, context):
    ...
    try:
        file_name = decode.parts[1].content.decode('utf-8')[:12]
        full_name = file_path + file_name + '.jpg'

        s3_response = s3.put_object(
            Bucket=BUCKET_NAME, Key=full_name, Body=decode.parts[0].content)
    except Exception as e:
        raise IOError(e)

    response['body'] = json.dumps(
        {'url': f'{"https://s3.ap-northeast-2.amazonaws.com/hknu-pptimage"}/{file_path}{file_name}.jpg'})
    return response
